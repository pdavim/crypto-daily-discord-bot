...
export function buildAlerts({
    ...
    // Existing alerts
    if (rsi != null && rsi > rsiOverbought) alerts.push("📉 RSI>70 (sobrecompra)");
    if (rsi != null && rsi < rsiOversold) alerts.push("📈 RSI<30 (sobrevenda)");
    if (prevRsi != null && rsi != null) {
        if (prevRsi > rsiOverbought && rsi <= rsiOverbought) alerts.push("📉 RSI cross-back ↓ (70→<70)");
        if (prevRsi < rsiOversold && rsi >= rsiOversold) alerts.push("📈 RSI cross-back ↑ (<30→>30)");
        if (prevRsi > rsiOverbought && rsi < rsiOverbought && rsi >= rsiMidpoint) alerts.push("🔄 RSI neutral (de >70)");
        if (prevRsi < rsiOversold && rsi > rsiOversold && rsi <= rsiMidpoint) alerts.push("🔄 RSI neutral (de <30)");
        if (prevRsi < rsiMidpoint && rsi >= rsiMidpoint) alerts.push("📈 RSI crossed 50↑ (momentum shift)");
        if (prevRsi > rsiMidpoint && rsi <= rsiMidpoint) alerts.push("📉 RSI crossed 50↓ (momentum shift)");
    }
    ...
    if (adxVal != null && adxVal >= adxStrongTrend) alerts.push("💪 ADX>25 (tendência forte)");
    ...
    if (avgVol && lastVol > volumeSpike * avgVol) alerts.push("🔊 Volume spike (>2x avg)");
    if (prevAtr != null && atr != null && atr > atrSpike * prevAtr) alerts.push("⚡ ATR spike (volatility)");
    ...
    if (heuristic != null && heuristic > heuristicHigh) alerts.push("🌟 Heuristic score very high");
    if (heuristic != null && heuristic < heuristicLow) alerts.push("⚠️ Heuristic score very low");
    ...
    if (stochK != null && stochK > stochasticOverbought) alerts.push("📉 Stochastic overbought");
    if (stochK != null && stochK < stochasticOversold) alerts.push("📈 Stochastic oversold");
    if (willr != null && willr > williamsROverbought) alerts.push("📉 Williams %R overbought");
    if (willr != null && willr < williamsROversold) alerts.push("📈 Williams %R oversold");
    if (cci != null && cci > cciOverbought) alerts.push("📉 CCI overbought");
    if (cci != null && cci < cciOversold) alerts.push("📈 CCI oversold");
    if (obv != null && prevObv != null && obv > prevObv * (1 + obvDelta)) alerts.push("📈 OBV bullish divergence");
    if (obv != null && prevObv != null && obv < prevObv * (1 - obvDelta)) alerts.push("📉 OBV bearish divergence");
